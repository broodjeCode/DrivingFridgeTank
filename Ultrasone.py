...
## The loop function that is started from the parent
	def run(self, UltrasoneData): # This function is called from Robot.py to start the Ultrasone thread.
		while True:
				...
				self.distance[0]=self.PrivGetDist(self.ECHO1, self.TRIG1, self.distance[0]) # get sensor 0 distance
				self.distance[1]=self.PrivGetDist(self.ECHO2, self.TRIG2, self.distance[1]) # get sensor 1 distance

				if self.debug:
					print "Distance: %s cm" % str(self.distance)
				UltrasoneData[0]=self.distance[0]	#float(self.distance)
				UltrasoneData[1]=25.44			# Hardcoded for now, I only have two sensors now.
